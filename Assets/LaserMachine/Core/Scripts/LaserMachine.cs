...
public class LaserMachine : MonoBehaviour {
    ...
    public void ConfigureForSingleShot(float maxDistance)
    {
        m_overrideExternalProperties = true;
        
        m_inspectorProperties.m_raysNumber = 1;
        m_inspectorProperties.m_rotate = false;
        m_inspectorProperties.m_angularRange = 0;
        m_inspectorProperties.m_maxRadialDistance = maxDistance;
        m_inspectorProperties.m_intermittent = false;
        
        // Re-initialize
        foreach(var element in elementsList)
        {
            if(element.transform != null) Destroy(element.transform.gameObject);
        }
        elementsList.Clear();
    ...
    void Initialize()
    {
        ...
        float angleStep = m_currentProperties.m_raysNumber > 1 ? m_currentProperties.m_angularRange / m_currentProperties.m_raysNumber : 0;        
        ...
        for (int i = 0; i < m_currentProperties.m_raysNumber ; i++)
        {
            ...
            if (m_currentProperties.m_raysNumber > 1)
